...
def test_configured_iters_are_a_floor_for_small_corpora():
    ...
    The floor still lifts a tiny corpus above its epoch count -- 24 > the 12
    steps two epochs would give -- but it is now bounded by lora.max_epochs.
    Unbounded, the 150 floor meant 25 epochs over six samples, which is what
    left every skill worker reciting its seed samples verbatim.
    """
    lora = {"epochs": 2, "batch_size": 1, "iters": 150, "max_iters": 2000}
    assert training.iters_for_corpus(lora, 6) == 24
    assert training.iters_for_corpus(lora, 6) > math.ceil(2 * 6)


def test_the_floor_cannot_exceed_max_epochs_over_the_corpus():
    """A floor large enough to memorise a small corpus is capped back."""
    lora = {"epochs": 2, "batch_size": 1, "iters": 150, "max_iters": 2000,
            "max_epochs": 4}
    for samples in (6, 7, 20, 26):
        iters = training.iters_for_corpus(lora, samples)
        assert iters <= 4 * samples, (samples, iters)
    # Large corpora are untouched: the floor never binds there anyway.
    assert training.iters_for_corpus(lora, 219) == 438
...
def test_max_iters_outranks_the_floor():
    ...
    max_iters must beat the floor even when the floor survives the max_epochs
    bound, so the cap is set below that bound here (4 epochs over 6 samples is
    24; the cap is 10).
    """
    lora = {"epochs": 2, "batch_size": 1, "iters": 300, "max_iters": 10}
    assert training.iters_for_corpus(lora, 6) == 10
    assert training.iters_for_corpus(lora, 0) == 10
...
def test_training_is_refused_when_memory_cannot_hold_it(monkeypatch):
    ...
    # 4.0 GB of weights + 2.6 x 4.35 GB of overhead at the table's own
    # reference window. The overhead is added, not multiplied — see
    # test_a_large_model_is_not_refused_a_run_it_has_already_completed.
    assert "15.3 GB" in message and "2.0 GB" in message


def test_a_large_model_is_not_refused_a_run_it_has_already_completed(monkeypatch):
    """The 14B fine-tune in train_14b_scrape_v4.log, replayed against the guard.

    Settings from training_data/_scrape_v4_14b.yaml: q+v keys, checkpointing,
    2k window, 8 layers. It ran 150 iterations to completion at a measured peak
    of 9.018 GB. The multiplicative form predicted 14.1 GB and would have
    refused it — silently turning off self-fine-tuning on the day the
    headmaster moved to the 14B.
    """
    monkeypatch.setattr(training, "_model_weight_bytes", lambda name: 7_850_000_000)
    monkeypatch.setattr(training, "free_ram_bytes", lambda: 11_000_000_000)
    config = {
        "model_name": "Qwen/Qwen3-14B-MLX-4bit", "gpu": {},
        "lora": {"grad_checkpoint": True, "max_seq_length": 2048,
                 "keys": ["self_attn.q_proj", "self_attn.v_proj"]},
    }

    need = training._training_need(config, 7_850_000_000)
    assert 9.018e9 < need < 11e9, f"{need/1e9:.2f} GB is not a safe estimate"
    assert training._memory_shortfall(config) is None


def test_the_eight_b_thresholds_the_table_was_measured_at_still_hold(monkeypatch):
    """Rewriting the overhead must not loosen the case it was calibrated on."""
    weights = 4_350_000_000  # the 4-bit 8B the table was measured against
    monkeypatch.setattr(training, "_model_weight_bytes", lambda name: weights)
    for checkpoint, keys, measured in [
        (True, ["self_attn.q_proj", "self_attn.v_proj"], 7.535e9),
        (False, ["self_attn.q_proj", "mlp.up_proj"], 15.583e9),
    ]:
        config = {"model_name": "some/model", "gpu": {},
                  "lora": {"grad_checkpoint": checkpoint, "keys": keys,
                           "max_seq_length": 3072}}
        need = training._training_need(config, weights)
        assert need >= measured, (
            f"estimate {need/1e9:.2f} GB is below the measured "
            f"{measured/1e9:.2f} GB peak — that is an OOM, not a retry")
