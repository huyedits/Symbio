...
def _built_in_golden_cases_only(tmp_path, monkeypatch):
    """Keep the user's golden_cases.json out of the unit tests.

    run_golden_set runs the built-in battery plus any user-defined extensions
    (constants.GOLDEN_CASES_FILE), but these tests script replies per built-in
    case id — an extension case has no scripted reply, so it errors and the
    totals shift. Pin the file into tmp_path so only the built-ins run.
    """
    monkeypatch.setattr(constants, "GOLDEN_CASES_FILE",
                        tmp_path / "golden_cases.json")
...
def _corpus(tmp_path, monkeypatch, samples):
    """Write `samples` (lists of messages) as a corpus and point golden at it."""
    train = tmp_path / "train.jsonl"
    train.write_text(
        "".join(json.dumps({"text": "rendered", "messages": m}) + "\n"
                for m in samples),
        encoding="utf-8")
...
_SAVE_NOTE_PROMPT = "Please remember that I prefer concise replies."


def test_find_corpus_contradictions_flags_samples_that_fail_their_own_case(
        tmp_path, monkeypatch):
    _corpus(tmp_path, monkeypatch, [
        _pair(_SAVE_NOTE_PROMPT,
              "Got it. <note title='Pref'>Prefers concise replies.</note>"),
        # Same prompt, no tag: nothing is saved, so the case it answers fails.
        _pair(_SAVE_NOTE_PROMPT, "Noted. I'll keep replies concise."),
        _pair("What is 2 + 2?", "4."),
    ])

    hits = golden.find_corpus_contradictions(_base_config())

    assert [(h.case_id, h.line_no) for h in hits] == [("save_note", 2)]
    assert "Noted." in hits[0].reply


def test_find_corpus_contradictions_ignores_prompts_no_case_owns(
        tmp_path, monkeypatch):
    _corpus(tmp_path, monkeypatch, [
        _pair("Tell me a joke.", "No tags here, just prose."),
    ])
    assert golden.find_corpus_contradictions(_base_config()) == []


def test_drop_corpus_contradictions_removes_only_those_lines(tmp_path, monkeypatch):
    good = "Got it. <note title='Pref'>Prefers concise replies.</note>"
    train = _corpus(tmp_path, monkeypatch, [
        _pair(_SAVE_NOTE_PROMPT, good),
        _pair(_SAVE_NOTE_PROMPT, "Noted, I'll be brief."),
        _pair("Open Chrome.", "Opening it in the browser for you."),
    ])

    dropped = golden.drop_corpus_contradictions(_base_config())

    assert dropped == {"save_note": 1, "open_app_command": 1}
    kept = train.read_text(encoding="utf-8").strip().splitlines()
    assert len(kept) == 1
    assert good in kept[0]
    # The pre-prune corpus is still on disk: dropping training data is not
    # something to do without a way back.
    backups = list(tmp_path.glob("train.jsonl.precontradiction.*"))
    assert len(backups) == 1
    assert len(backups[0].read_text(encoding="utf-8").strip().splitlines()) == 3


def test_drop_corpus_contradictions_is_a_no_op_on_a_clean_corpus(
        tmp_path, monkeypatch):
    train = _corpus(tmp_path, monkeypatch, [
        _pair(_SAVE_NOTE_PROMPT,
              "Got it. <note title='Pref'>Prefers concise replies.</note>"),
    ])
    before = train.read_text(encoding="utf-8")

    assert golden.drop_corpus_contradictions(_base_config()) == {}
    assert train.read_text(encoding="utf-8") == before
    assert not list(tmp_path.glob("train.jsonl.precontradiction.*"))
...
def test_guarded_train_teaches_cases_already_failing_at_baseline(tmp_path, monkeypatch):
    """A case that fails *before* training can never appear in the regression
    set, so without this it gets no remedy samples from any round — it just
    stays broken and keeps printing FAIL. Measured live: open_app_command and
    save_note both failed baseline, and every later /train left them there."""
    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "adapters")
    monkeypatch.setattr(constants, "TRAIN_FILE", tmp_path / "train.jsonl")
    _write_adapter("original")
    ...
    # save_note and open_app_command are already failing at baseline and stay
    # failing after: no regression, so only the new path can teach them.
    failing = {"greeting": True, "save_note": False, "open_app_command": False}
    monkeypatch.setattr(golden, "run_golden_set",
                        lambda *a, **k: golden.GoldenResult(dict(failing), {}))

    remedied = []

    def spy(case_ids, tokenizer, system_prompt, cfg, **kwargs):
        remedied.append((list(case_ids), kwargs.get("copies")))
        return len(case_ids) * (kwargs.get("copies") or 1)

    monkeypatch.setattr(golden, "append_golden_remedy_samples", spy)

    session = _make_session(config, monkeypatch, [])

    assert session._guarded_train() is True
    # Only the failing cases, and only once — before training, so the round
    # that follows is the one that learns them.
    assert remedied == [
        (["open_app_command", "save_note"],
         config["learn"]["golden_retry_samples_per_case"]),
    ]


def test_guarded_train_baseline_teaching_can_be_disabled(tmp_path, monkeypatch):
    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "adapters")
    monkeypatch.setattr(constants, "TRAIN_FILE", tmp_path / "train.jsonl")
    _write_adapter("original")
    ...
    config = _base_config()
    config["learn"]["golden_set_enabled"] = True
    config["learn"]["golden_teach_baseline_failures"] = False

    monkeypatch.setattr(training, "run_training",
                        lambda cfg, iters=None: _write_adapter("trained-ok") or True)
    monkeypatch.setattr(golden, "run_golden_set",
                        lambda *a, **k: golden.GoldenResult({"save_note": False}, {}))

    remedied = []
    monkeypatch.setattr(golden, "append_golden_remedy_samples",
                        lambda *a, **k: remedied.append(a) or 3)
