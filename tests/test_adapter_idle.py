...
from symbio import config as symbio_config
...
# ---- an adapter directory with no weights in it ----

def test_a_directory_with_config_and_weights_counts_as_an_adapter(tmp_path, monkeypatch):
    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "adapters")
    _write_adapter()
    assert symbio_config.adapter_weights_present()


def test_an_orphan_config_left_by_a_killed_run_does_not(tmp_path, monkeypatch):
    """A run that was OOM-killed mid-train leaves adapter_config.json behind
    with no safetensors beside it. mlx_lm's load(adapter_path=...) raises on
    that, and in the headmaster wake path the load sits under one broad
    except -- so the model did not wake at all."""
    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "adapters")
    constants.ADAPTER_DIR.mkdir(parents=True)
    (constants.ADAPTER_DIR / "adapter_config.json").write_text("{}")

    assert not symbio_config.adapter_weights_present()


def test_weights_without_a_config_do_not_either(tmp_path, monkeypatch):
    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "adapters")
    constants.ADAPTER_DIR.mkdir(parents=True)
    (constants.ADAPTER_DIR / "adapters.safetensors").write_bytes(b"weights")
...
def test_a_missing_directory_is_not_an_adapter(tmp_path, monkeypatch):
    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "never-made")
    assert not symbio_config.adapter_weights_present()


def test_the_check_follows_a_redirected_adapter_dir(tmp_path, monkeypatch):
    """Resolved through the module, not a name imported at import time: the
    tests and the worker harnesses both redirect constants.ADAPTER_DIR at
    runtime, and a from-import would have frozen the real one."""
    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "a")
    _write_adapter()
    assert symbio_config.adapter_weights_present()

    monkeypatch.setattr(constants, "ADAPTER_DIR", tmp_path / "b")
    assert not symbio_config.adapter_weights_present()
