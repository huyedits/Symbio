...
def test_trigger_section_is_not_part_of_the_steps(skill_entry, tmp_path, monkeypatch):
    """Otherwise a skill's own correct answer fails its own golden check.

    The note carries a `## Triggers` block of routing keywords and example
    phrasings. Graded as if it were procedure, those words dominate the answer
    key: fix_wifi's real answer scored 0.15 coverage against a 0.5 floor, so
    the derived checks passed nothing and the worker regression gate was
    effectively off for every skill with triggers.
    """
    notes = tmp_path / "notes"
    notes.mkdir()
    monkeypatch.setattr(constants, "NOTES_DIR", notes)
    (notes / "skill_fix_wifi.md").write_text(
        f"# Skill: {SKILL_NAME}\n\n{STEPS}\n\n"
        "## Triggers\n\n"
        "Keywords: wifi, wireless, no internet, connection dropped, router\n\n"
        "Examples:\n\n- my wifi is broken\n- the wifi dropped out again\n",
        encoding="utf-8")

    steps = skill_eval.skill_steps(skill_entry)
    assert steps.strip() == STEPS
    assert "Triggers" not in steps and "router" not in steps

    # The answer key must actually accept the answer.
    keywords = skill_eval._keywords(skill_eval._step_body(steps))
    assert skill_eval.coverage(STEPS, keywords) >= skill_eval.GOLDEN_COVERAGE_FLOOR
