...
def test_a_tag_opener_left_dangling_at_the_end_is_never_shown():
    """Live: a reply that was 25 <cmd> tags plus a trailing '<' printed
    "Caine   : <" as the entire visible answer. _first_ambiguous_lt holds
    that '<' waiting for a tag name that never arrives, and strip_tool_tags
    leaves it alone because there is no tag there to recognize — so finish()
    has to drop the fragment itself."""
    for tail in ("<", "<c", "<cm"):
        full = "<cmd>ls</cmd>" * 25 + tail
        stripper = tooling.StreamingStripper()
        out = "".join(stripper.feed(ch) for ch in full) + stripper.finish()
        assert out == "", (tail, out)


def test_a_dangling_opener_does_not_take_the_answer_with_it():
    stripper = tooling.StreamingStripper()
    full = "You're using Colemak. <cmd>defaults read"
    out = "".join(stripper.feed(ch) for ch in full) + stripper.finish()
    assert out.strip() == "You're using Colemak."
...
# ---- the reply prefix belongs to an actual reply ----
#
# Live: every turn whose whole reply was a tool tag printed a bare
# "Caine   : " line above the [Tool]/[Blank] notice — an answerless prompt
# that reads as the assistant having said nothing back. Nothing was written
# to that line (the spinner's stop() clears it), so no newline is owed
# either; the consolidated print below the stream handles the real reply.

def test_a_reply_that_is_only_a_tool_tag_prints_no_prefix(monkeypatch):
    session, _ = _make_session(monkeypatch, ["<cmd>ls</cmd>"])
    written = []
    session.stream_chunk_fn = written.append

    messages = [{"role": "system", "content": "SYS"}, {"role": "user", "content": "hi"}]
    raw, streamed_live = session._generate_reply(messages, chunk_prefix="Caine   : ")

    assert streamed_live is False
    assert written == [], written


def test_a_real_reply_still_gets_its_prefix_and_a_closing_newline(monkeypatch):
    session, _ = _make_session(monkeypatch, ["You're using Colemak."])
    written = []
    session.stream_chunk_fn = written.append
    ...
    assert streamed_live is True
    assert "".join(written).startswith("Caine   : ")
    assert "".join(written).endswith("\n")
...
# ---- reasoning streams live instead of landing in one paste ----
#
# The thinking block used to be held whole: feed() returned "" for every token
# from the open delimiter until the close arrived, so a several-hundred-token
# think showed as a spinner and then the entire block at once. That is the
# slowest part of the turn rendering as a freeze. It now streams like the
# answer does, with only a possible partial close delimiter held back.

O, C = tooling._QWEN_THINK_OPEN, tooling._QWEN_THINK_CLOSE


def _drive(chunks, show_reasoning=True):
    """Feed chunks one at a time; return [(text, is_reasoning), ...]."""
    stripper = tooling.StreamingStripper(show_reasoning=show_reasoning)
    out = []
    for chunk in chunks:
        text = stripper.feed(chunk)
        if text:
            out.append((text, stripper.chunk_is_reasoning))
    tail = stripper.finish()
    if tail:
        out.append((tail, stripper.chunk_is_reasoning))
    return out


def test_reasoning_streams_token_by_token():
    out = _drive([O, "Okay", ", let's", " see.", C, "\n", "Hello", "!"])
    # Every reasoning token came out as its own chunk, not one block at the end.
    assert [t for t, r in out if r] == ["  [Reasoning] Okay", ", let's", " see.", "\n"]
    assert "".join(t for t, r in out if not r) == "Hello!"
    print("test_reasoning_streams_token_by_token passed")


def test_reasoning_marker_is_emitted_exactly_once():
    out = _drive([O, "a", "b", "c", C, "Answer."])
    marked = [t for t, _ in out if tooling.REASONING_MARKER in t]
    assert len(marked) == 1 and marked[0].startswith(tooling.REASONING_MARKER), out
    print("test_reasoning_marker_is_emitted_exactly_once passed")


def test_split_close_delimiter_never_flashes():
    # The close delimiter arrives one fragment at a time; none of it may show.
    out = _drive([O, "think", "ing", "</", "th", "ink>", "Answer."])
    assert not any(C in t or "</th" in t for t, _ in out), out
    assert "".join(t for t, r in out if not r) == "Answer."
    print("test_split_close_delimiter_never_flashes passed")


def test_chunk_is_reasoning_marks_every_reasoning_chunk():
    # This is what keeps the reply prefix ("Caine   : ") off the middle of a
    # thought: only the first reasoning chunk carries REASONING_MARKER, so the
    # caller cannot sniff for it and has to be told.
    out = _drive([O, "one", " two", " three", C, "Answer."])
    assert [r for _, r in out] == [True, True, True, True, False], out
    print("test_chunk_is_reasoning_marks_every_reasoning_chunk passed")


def test_hidden_reasoning_still_shows_nothing():
    out = _drive([O, "secret", " plan", C, "Answer."], show_reasoning=False)
    assert out == [("Answer.", False)], out
    print("test_hidden_reasoning_still_shows_nothing passed")


def test_unclosed_reasoning_block_streams_and_yields_no_answer():
    # The adapter over-thinks open-ended prompts and sometimes never closes the
    # block. The text still streams (it is all the user gets), and nothing is
    # mislabelled as the answer.
    out = _drive([O, "thinking", " forever"])
    assert all(r for _, r in out), out
    assert "".join(t for t, r in out if not r) == ""
    print("test_unclosed_reasoning_block_streams_and_yields_no_answer passed")


def test_whole_block_in_one_chunk_still_works():
    out = _drive([O + "one shot" + C + "Hi."])
    assert out == [(f"{tooling.REASONING_MARKER}one shot\n", True), ("Hi.", False)], out
    print("test_whole_block_in_one_chunk_still_works passed")


def test_reply_with_no_reasoning_is_untouched():
    out = _drive(["Just ", "an ", "answer."])
    assert out == [("Just ", False), ("an ", False), ("answer.", False)], out
    print("test_reply_with_no_reasoning_is_untouched passed")


def test_partial_suffix_len():
    assert tooling._partial_suffix_len("abc</th", C) == 4
    assert tooling._partial_suffix_len("abc<", C) == 1
    assert tooling._partial_suffix_len("abc", C) == 0
    # A complete marker is not a partial one; callers search for it first.
    assert tooling._partial_suffix_len("abc" + C, C) == 0
    print("test_partial_suffix_len passed")
