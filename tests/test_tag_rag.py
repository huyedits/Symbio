...
# tag_rag.py is an optional drop-in at the repo root: rag.py and chat.py guard
# its absence and silently disable the tag index, so this module must skip
# (not error) when it isn't there. Otherwise it breaks collection for the
# whole suite the moment the file is absent.
pytest.importorskip("tag_rag")
from tag_rag import TagIndex  # noqa: E402  (guarded by importorskip above)
