...
def test_stream_sender_sends_placeholder_on_slow_first_chunk(stream_loop):
    ...
    # Comfortably longer than the 50ms timeout. 0.15 lost this race once in a
    # full-suite run on a loaded machine — a wall-clock margin of 3x is not a
    # margin when 1900 other tests are competing for the CPU.
    time.sleep(0.5)
    assert bot.send_message.await_count == 1
