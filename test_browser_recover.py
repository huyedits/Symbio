...
class FakeBrowser:
    ...
    def type_text(self, text, selector="", press_enter=False):
        # Mirrors BrowserSession.type_text, selector included. Without it the
        # dispatch's selector argument raised TypeError, the generic handler
        # turned that into "failed unexpectedly", and the reopen-and-retry
        # recovery this file exists to test never ran.
...
class Session:
    ...
    # Copied by name, so anything _dispatch_tool starts calling has to be
    # added here too — otherwise the branch dies with AttributeError and the
    # test reports it as the browser failing.
    _no_effect_note = chat.ChatSession._no_effect_note
...
def no_peek(monkeypatch):
    ...
    # Signature must track the real helper — it takes the config now, so it can
    # honour agent.browser_peek_chars instead of a hardcoded 1500.
    monkeypatch.setattr(chat, "_browser_peek", lambda _b, _cfg=None: "")
